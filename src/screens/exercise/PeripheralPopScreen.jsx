...
// Eye Exercise: Peripheral Pop (Fixed)
// ============================================================================

import React, { useState, useEffect, useRef, useCallback } from 'react';
import { View, Text, TouchableOpacity, StyleSheet, Dimensions, Modal, Alert } from 'react-native';
...
const StimulusDot = React.memo(({ id, style, onRemove }) => {
  ...
  return (
    <TouchableOpacity
      ...
      activeOpacity={0.7}
      accessible={true}
      accessibilityLabel={`stimulus-dot-${id}`}>
      {/* ensure there is always a React element child — never a raw string */}
      <View />
    </TouchableOpacity>
  );
});
...
const PeripheralPopScreen = ({ navigation }) => {
  ...
  const createPeripheralDot = useCallback(() => {
  ...
  useEffect(() => {
    ...
    sessionTimerRef.current = setTimeout(() => {
      setIsModalVisible(true); // Show modal after 60s
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.gameArea}>
        ...
        {dots.map((dot) => (
          <StimulusDot key={String(dot.id)} id={dot.id} style={dot.style} onRemove={removeDot} />
        ))}
      </View>
